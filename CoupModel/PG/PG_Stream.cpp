...
bool PG_Stream::ReadPGStartHeaderFromStream(fstream *pReadOnlyStream) {
	...
	if (nrep == 4 && !Check_PGFileSize(pReadOnlyStream, OrgRec, pg.Shape.NumVar, nrep)) {
		nrep = 1;
...
bool PG_Stream::WriteDescriptionsToStream(fstream * pWriteEnabledStream)
{
...
bool PG_Stream::WritePGFileStartHeader(size_t nrecords, size_t nrepititions, size_t InputRepNo, fstream *pPGOutStream)
{
	...
	if (pPGOutStream==nullptr)
		pPGOutStream = &m_MainPGStreamReadWrite;
...
bool PG_Stream::ReadNewBuffer(size_t Buffer) {
	std::streamsize newpos;
	if (Buffer == 0)
		newpos = pg.Shape.RecordSize;
	else if (Buffer <= pg.status.b.NumberOfBuf)
		newpos = pg.Shape.RecordSize*(Buffer - 1)*pg.status.b.buf_Size + pg.Shape.RecordSize;
	else if (Buffer <= pg.Shape.NumRepetitions)
		newpos = pg.Shape.RecordSize*(Buffer - 1)*pg.status.b.buf_Size + pg.Shape.RecordSize;
	else
		return false;
	m_MainPGStreamReadWrite.seekg(newpos, ios::beg);
...
bool PG_Stream::WritePGFile()
{
	...
	if (!m_MainPGStreamReadWrite.good()) return false;
	//if(pg.status.CompleteFileRead==false&&m_DataInFocus==0) return true;
...
bool PG_Stream::WriteNewPGFileAs(string newfilename, bool newfile)
{
	...
	if (!m_MainPGStreamReadWrite.is_open()) {
		...
		if (pg.Shape.NumRepetitions>1) return false;
		m_OnlyNewFile = true;
