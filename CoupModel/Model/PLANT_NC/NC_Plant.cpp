...
public:	 
	...
	~ZadokOutCreateFile(void) {};
	double operator() (double Zadok, int DayNum) {
		if (Zadok >= 0.&&Zadok < 10.) {
			if (shift) {
				z0 = double(DayNum + 0.0001);
				shift = false;
			}
			return z0;
		}
		else if (Zadok >= 10.&&Zadok < 30.) {
			if (!shift) {
				z0 = double(DayNum + 0.0001);
				shift = true;
			}
	...
	void Reset(bool clearstrings) {
		...
		ZadokDates.clear();
		outfile.close();
		outfile.open("calCSIRO_Results_CoupModel_LisbetLewan.txt", std::ios::app);
		if(clearstrings) zadokoutstrings.clear();
	...
	double Update(double Zadok, int DayNum, size_t ActMin ,int FileIndex) {
		...
		auto FixDatum = [&](int Year, int DayNum) {
			...
			std::string out = datumstr.substr(5, 2) + "/" + datumstr.substr(8, 2) + "/" + datumstr.substr(0, 4);
			return out;
		};
		auto MakeLine = [&]() {
			string outline;
			if (ZadokDates.size() == 5) {
				outline = ZadokDates[0].substr(6, 4) + "\t";
				outline += sitename[FileIndex - 1] + "\t";
				for (size_t i = 0; i < 5; i++) { outline += ZadokDates[i]; if (i < 4) outline += "\t"; }
		...
		if (Zadok >= 0.&&Zadok < 10.) {
			...
			if (Zadok <= .01) {
				if (ZadokDates.size() == 0) ZadokDates.resize(1);
				ZadokDates[0] = FixDatum(Year, DayNum);
			}
			return z0;
		}
		else if (Zadok >= 10.&&Zadok < 30.) {
			if (!shift) {
				z0 = double(DayNum + 0.0001);
				if (ZadokDates.size() == 1) ZadokDates.resize(2);
				ZadokDates[1] = FixDatum(Year, DayNum);
				shift = true;
			}
			return z0;
		}
			...
			if (shift) {
				z0 = double(DayNum + 0.0001);
				shift = false;
				if (ZadokDates.size() == 2) ZadokDates.resize(3);
				ZadokDates[2] = FixDatum(Year, DayNum);
			}
			return z0;
		}
		else if (Zadok >= 65.&&Zadok < 90.) {
			if (!shift) {
				z0 = double(DayNum + 0.0001);
				shift = true;
				if (ZadokDates.size() == 3) ZadokDates.resize(4);
				ZadokDates[3] = FixDatum(Year, DayNum);
			}
			...
			if (shift) {
				z0 = double(DayNum + 0.0001);
				shift = false;
				if (ZadokDates.size() == 4) {
					ZadokDates.resize(5);
					ZadokDates[4] = FixDatum(Year, DayNum);
					MakeLine();
				}
			}
			return z0;
	...
	void   CreateFileOutput(size_t ActualNoMulRun) {
		...
		if ((ActualNoMulRun) % 3 == 0 && zadokoutstrings.size() > 2) {
...
bool NC_Plant::Ini()	{
	...
	if (ZadokScale_Sw(ZadokScale) ==ZadokScale_Sw::withOutPut2File) {
		if(pZadokOutCreateFile==nullptr) pZadokOutCreateFile = new ZadokOutCreateFile();
	...
	auto SetInitialValueforStateVariables = [&]() {
		...
		/*	C_TotAbovePlantLitter = 0.0;
			N_TotAbovePlantLitter = 0.0;
			C_TotPlantLitter = 0.0;
			N_TotPlantLitter = 0.0;
			N_TotFungiLitter = 0.0;
			C_TotFungiLitter = 0.0;
			N_Tot_FungiUptake = 0.0; //AM 200911 added variables*/ //HH
			...
			for (size_t jj = 0; jj < m_pModelStructure->m_NumActPlantElements; jj++) {
				for (size_t j = _LEAF; j < NUM_PLANT_FUNGI_COMP; j++) {
					...
					for (size_t i = 0; i < pPlant->NumPlants; i++) {
						if (Emergence_Switch_Sw(Emergence_Switch) == Emergence_Switch_Sw::off)
	...
	auto SetSizeofVariablesandOtherSettings = [&]() {
		...
		for (size_t i = 0; i < Initial_GSI.size(); i++)    if (Initial_GSI[i] >= 1.) Emergence_Done[i] = true;
		...
		PCO2B.assign(pPlant->NumPlants, CO2_A*P_Surface / 1.E6);
		ICountDay = 0;
	...
	auto SetManagementFunctors = [&]() {
		...
		if (!f_GrowthTemp) {
			f_GrowthTemp = GrowingSumF(T_Thres_Emerg, pAtm->Latitude);
			f_DormingTemp = GrowingSumF(T_Thres_Emerg, T_Thres_Dormancy, pAtm->Latitude);
		}
		if (ZadokScale_Sw(ZadokScale) >= ZadokScale_Sw::on) {
			f_Zadok = ZadokFunctionFromGSI(ZadokRangeGSI2);
		};
	};
		...
		auto InflowAboveGroundComponentsToSoilHorizon = [&](size_t jj_element, size_t i_plant, size_t i_soil_layer, double Fraction) {
			for (size_t j = _LEAF; j <= _OLD_STEM; j++) {
				if (P_LitterFall[j][jj_element][i_plant] >= 0. && P_LitterFall[j][jj_element][i_plant] < 100000.) {
					...
					if (Fraction < 1.) {
						if (jj_element == _C_) InFlow(pNCSoil->O_State[_L1][_C_][i_soil_layer], P_LitterFall[j][jj_element][i_plant] * (1 - Fraction));
						else if (jj_element == _N_) InFlow(pNCSoil->O_State[_L1][_C_][i_soil_layer], P_LitterFall[j][jj_element][i_plant] * (1 - Fraction));
						else if (jj_element == _P_) InFlow(pNCSoil->O_State[_L1][_C_][i_soil_layer], P_LitterFall[j][jj_element][i_plant] * (1 - Fraction));
...
void NC_Plant::PlantStage_CNRatio_PhysSize() {
	...
	for (size_t jj = 0; jj < m_pModelStructure->m_NumActPlantElements; jj++) {
		for (size_t j = 0; j < NUM_PLANT_COMP; j++) P_LitterFall[j][jj].assign(pPlant->NumPlants, 0.);
...
auto Growth_Stage = [&](size_t index){
	...
	if (GrowthStageIndex[index]<0 && Max_GSI[index]>-1.) {  //// -1 Seed not in soil
		if (SowingDayNo[index] > 0 && Sowing_Switch_Sw(Sowing_Switch) == Sowing_Switch_Sw::Fixed_dayNo) {                            //// 0 Seed in Soil
			...
			if (out >= 0.) f_Sowing = PlantTempSum(T_Thres_Sowing, T_Sum_Sowing); //SowingSum[index]=0.;
		}
	}
	else if (GrowthStageIndex[index]<1.&&Max_GSI[index]>0.) { // // 1 emergence
		if (EmergenceDayNo[index] > 0 && Emergence_Switch_Sw(Emergence_Switch) == Emergence_Switch_Sw::Fixed_dayNo) {
		...
		else if (Emergence_Switch_Sw(Emergence_Switch) == Emergence_Switch_Sw::Temperature_Sum) {
			out = 0. + f_EmergenceWithMoistControl(index, NDrivTAir, NDrivTheta[0], T_Step);
			if (out >= 1.) {
				f_Emergence = PlantTempSum(T_Thres_Emerg, T_Sum_Emerg);
				f_EmergenceWithMoistControl = PlantTempSum(T_Thres_Emerg, T_Sum_Emerg, p_Soil_WaterF->M_Wilting[0], MoistureRangeEmergence);
				Emergence_Done[index] = false;
	...
	else if (GrowthStageIndex[index]<2.&&Max_GSI[index]>1.) { //
		out = 1. + f_GrainStart(index, m_pModelInfo->DayLength, NDrivTAir, T_Step);
		if (out >= 2) {
			f_GrainStart = GrainStart(GrainSI_Step, GrainSI_StepDay, GrainSI_ThresDay, GrainSI_StepTemp, GrainSI_ThresTemp);
		}
	}
	else if (GrowthStageIndex[index]<3.&&Max_GSI[index]>2.) {		// Grain filling start
...
auto Zadok_Index = [&](size_t i) {
	if (ZadokScale_Sw(ZadokScale) >= ZadokScale_Sw::on&&GrowthStageIndex[i] >= 0.) {
...
CN_RatioPlant();
	...
	for(size_t i=0; i<P_State[_LEAF][_N_].size(); i++) {
		...
		GrowthTempSum[i] = f_DormingTemp(i, NDrivTAir, m_pModelInfo->JDayNum, T_Step, &DormingTempSum[i], &SpringDev);
		ConditionalUpdateOfPlantAge(i);
		if(Growth_Sw(Growth)>=Growth_Sw::Water_use_efficiency) {		
			if(GrowthStageIndex[i]>=1&& GrowthStageIndex[i]<1.1&&Min_GSI[i]<=1.&&P_State[_MOBILE][_C_][i]<0.01&&!Emergence_Done[i]&&Max_GSI[i]>1.) { //Emergence
				EmergenceAndRelatedUpdates(i);
			}  				
			else if(GrowthStageIndex[i]>1&& GrowthStageIndex[i]<=2&&P_State[_MOBILE][_C_][i]>0.001&&SpringDev) 
	...
	if(Growth_Sw(Growth)== Growth_Sw::Logistic_function) {	// Logistic growth
		...
		for(size_t i=0; i<GPP_Current.Temp.size(); i++) {
			if (GrowthStageIndex[i] >= 1.) {
				...
				if(NDrivRadInShort>0.) GPP_Current.Light[i] = RadPlantAdsorbed[i]/NDrivRadInShort;
...
void NC_Plant::Cleaning_Harvest_Events() {
	...
	for(size_t i=0; i<P_ReAllocation[0][0].size();i++) {
		if(Cleaning>0) {
		    DoCleaning=false;
		    if(Cleaning==1) 
		        YearToClean=true;
		       ...
		       // Do jj=1, Num_CleaningYears
				for(size_t jj=0; jj<CleaningYears.size();jj++) {
		            if(int(CleaningYears[jj])==int(SimPlantYearAge[i]))
		                YearToClean=true;	            
				}
		    }
		    if(YearToClean) {
		      //  Do JJP=1, Num_CleaningDays
				for(size_t jjp=0; jjp<CleaningDays.size(); jjp++) {
					if(int(CleaningDays[jjp])==m_pModelInfo->JDayNum&&CleaningPlantIndex[jjp]-1==i) {
		                DoCleaning=true;
		                IndexClean=i;
		...
		if(Harvest_Day_Sw(Harvest_Day)==Harvest_Day_Sw::Fixed_Multiple_Days) {
		    ...
		    for(size_t jjp=0; jjp<SimPlantYearAge.size(); jjp++) { //Do JJP=1, NumPlants
				for(size_t jj=0; jj<HarvestYears.size(); jj++) {   //Do jj=1, Num_HarvestYears		
					...
					else if(size_t(HarvestYearPlantIndex[jj]-0.01)==i&&jjp==i&&int(SimPlantYearAge[jjp])==int(HarvestYears[jj])) {
						DoHarvest=true  ;
						if(jj==HarvestYears.size()-1  ) DoFinalHarvest=true;
    				}
				}
		         ...
		         // Do jj=1, Num_HarvestDays  
				  for(size_t jj=0; jj<HarvestDays.size();jj++) {	
						if(PlantsToHarvest==0&&m_pModelInfo->JDayNum==int(HarvestDays[jj])) {
							HarvestDayNo[i]=m_pModelInfo->JDayNum;  
							if(jj<HarvestDays.size()-1) DoFinalHarvest=false;
						}
						else if(size_t(HarvestDayPlantIndex[jj]-0.01)==i&&m_pModelInfo->JDayNum==int(HarvestDays[jj])) {
							HarvestDayNo[i]=m_pModelInfo->JDayNum;
							if(jj<HarvestDays.size()-1) DoFinalHarvest=false;
						}
					}
   		    } 
   		    if(HarvestDayNo[i]==-1) HarvestToBeDone[i]=true;  
		}
   		else if(Harvest_Day_Sw(Harvest_Day)==Harvest_Day_Sw::Estimated_GSI) {
   		    HarvestDayNo[i]=0;
   		    if(GrowthStageIndex[i]>=4)
                HarvestDayNo[i]=m_pModelInfo->JDayNum;		   		      		    
   		}
   		else if(Harvest_Day_Sw(Harvest_Day)==Harvest_Day_Sw::Estimated_LAI/**&&i==1&&pMod->JDayNum>=270*/&&SimPlantYearAge.front()<(Max_Plant_LifeTime.front()-10)) {
   		    HarvestDayNo[i]=-1;
           // if(SimPlantYearAge.front()+8>PreviousThinningYear) {       		
   		        if(pPlant->SimLeafAreaIndex[i]>=HarvestLAIValue[i]){
   		            HarvestDayNo[i]=m_pModelInfo->JDayNum;
		    ...
		    if(DoHarvest) {
				...
				for(size_t jj=0; jj<HarvestDays.size();jj++) {
		            if(PlantsToHarvest==0&&m_pModelInfo->JDayNum==int(HarvestDays[jj])) 
		                HarvestDayNo[i]=m_pModelInfo->JDayNum;
		            else if(HarvestDayPlantIndex[jj]==i&&m_pModelInfo->JDayNum==int(HarvestDays[jj])) 
                        HarvestDayNo[i]=m_pModelInfo->JDayNum;  		                
		         }
		          //EndDo
   		    }    		    
		...
		// At Harvest
		if(((GrowthStageIndex[i]==4&&(Harvest_Day_Sw(Harvest_Day)==Harvest_Day_Sw::Estimated_GSI||
			Harvest_Day_Sw(Harvest_Day)==Harvest_Day_Sw::In_Year_Table))||
			(HarvestDayNo[i]==m_pModelInfo->JDayNum&&Harvest_Day_Sw(Harvest_Day) != Harvest_Day_Sw::Estimated_GSI &&
				Harvest_Day_Sw(Harvest_Day) != Harvest_Day_Sw::In_Year_Table))&&HarvestToBeDone[i]) {		// Harvest day, LK 020827 Updated bugs and added harvest from old
		    ResetHarvestToBeDoneDay[i]=min(int(m_pModelInfo->JDayNum+1),365);
            HarvestDoneThisYear[i]=true;
			if(Grazing_Sw(Grazing)==Grazing_Sw::off) 		    
				At_Harvest(false);
		    else if(HarvestToBeDone[i]&& Grazing_Sw(Grazing) == Grazing_Sw::Continous)  // When Grazing equal 2 14 May 2007   		
    			At_Harvest(true);
    		   
	////// For Grazing = 2 above		
		
		}    
		else if(SimPlantAge[i]>=Max_Plant_LifeTime[i]*365.-150.&&m_pModelInfo->JDayNum==1&&pAtm->Latitude>0.){
		        if(TerminatingHarvest==1) {
		            if(i==1) {
                        OK=TerminatingHarvestFunction(i);
                        HarvestToBeDone[i]=false	;
						SimPlantAge.front()=0;
                    }
				}
		        else {
				    OK=KillAboveGroundPlant(i);
				    OK=KillBelowGroundPlant(i);
				    HarvestToBeDone[i]=true;
				}
	...
	//			GrainStartSum[i]=0.;
		...
		else if(SimPlantAge[i]>=Max_Plant_LifeTime[i]*365.-150.&&m_pModelInfo->JDayNum==180&&pAtm->Latitude<=0.){
		        if(TerminatingHarvest==1) ;
		...
		else if(DoCleaning&&CleaningToBeDone&&IndexClean==i) 
			At_Cleaning();
		else {
		...
		if(HarvestDayNo[i]==m_pModelInfo->JDayNum+1&&HarvestToBeDone[i]==false) 
			HarvestToBeDone[i]=true;
		else if(ResetHarvestToBeDoneDay[i]==m_pModelInfo->JDayNum&&HarvestToBeDone[i]==false) {
		    HarvestToBeDone[i]=true	;
		}
	...
	for (size_t i = 0; i < P_Harvest[_GRAIN][_C_].size(); i++) {
		for (size_t j = 0; j < m_pModelStructure->m_NumActPlantElements; j++) {
			for (size_t jj = 0; jj < NUM_PLANT_COMP; jj++) {
				...
				P_LitterFall[jj][j][i] = CleaningDegree[IndexClean] * P_State[jj][j][i] * 1. / T_Step;
			}
		}
	}
...
void NC_Plant::MobilePool() {
	 for(size_t i=0; i< P_ReAllocation[LEAF_MOBILE][_N_].size();i++) {
		...
		if(SimPlantAge[i]<Max_Plant_LifeTime[i]*365.&&Winter_Regulation>0&&N_ReAllocation>0) {		//LK 020524
...
void NC_Plant::Summation_Fluxes() {
	...
	for (size_t jj = 0; jj < m_pModelStructure->m_NumActPlantElements; jj++) {
		...
		if (sum(P_Harvest[_GRAIN][jj]) > 0.) YearGrainHarvest[jj] = sum(P_Harvest[_GRAIN][jj]);

		if (TotHarvest[jj] > 0) YearTotalHarvest[jj] = TotHarvest[jj];
