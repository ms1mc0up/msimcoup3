...
void SimModule::Message(string str) {
	...
	size_t v=str.size();
	//v=(m_pModelMap)->AddToRunReport(str);
...
#ifdef COUPSTD
	if (m_NumErrors < 6) 
		cout << "Simulation Error "+STD_ItoAscii(m_NumErrors);
...
void SimModule::Define(Func* ptr,size_t opt, size_t ftype,SimModule *mClass, string name)
{

   ptr->Def(ModuleNo,opt, ftype, mClass, name, pFunc, m_pModelMap);
