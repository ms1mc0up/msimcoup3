...
bool Potential_Transpiration::Def()
{
	...
	Define(pPs, "EPMaxRate", 4., WATERFLOW_UNIT, "PlantType = 1|PotTranspInput = 0", 0.1, 20., 1., 10.); ps_vec.push_back(pPs);
	...
	Define(pPs, "CondRis", 5.E6, HEATFLOW_UNIT, "RSMethod >1|PlantType = 2", 1.E6, 500.E6);
	...
	Define(pP, pNumPlants, "Conduct Ris", initv, HEATFLOW_UNIT, "PlantType >= 3", 1.E6, 500.E6);
	mTab_EvapMulti.push_back(pP);
   ...
   if(m_pModelStructure->PlantType==1&&m_pModelStructure->Evaporation>=1) {
	  ...
	  if (RSMethod >= 2) {
		  ...
		  if (ConductanceCO2_Dependence == 0)
			  ...
			  ResSurfVar = f_CanopyResistance(f_Conductance(Winter, pMet->RadInShort, pMet->VapourPDef)*f_Conductance_CO2(pNC_Plant->CO2_A*1.E6), LAI_Total);
	  }
...
double Potential_Transpiration::Lohammar_LL(size_t index) {
	...
	if (ConductanceCO2_Dependence == 1) {
		scaleCoef = f_Conductance_CO2(pNC_Plant->CO2_A*1.E6);
	}
