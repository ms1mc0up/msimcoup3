...
//		Define(pSw, "AmmoniaVolatilisation",0,"off|on","Nitrogen and Carbon > 0");
	...
	Define(pPs, "Delta Power Coef pH", 0.27, NO_UNIT, "Phosphorus > 0", 0.027, 1.); 
	
	pPs = new Ps(&Weath_pH0, TOTPROFILE, MINERAL_P, NOFPROC, WEATHERING, NORMAL); ps_vec.push_back(pPs);
	Define(pPs, "pH Optimal", 7, NO_UNIT, "Phosphorus > 0", 5, 10., 3., 12.); 
	...
	Define(pPs, "Weathering Rate Coef", 1.E-6, NO_UNIT, "Phosphorus > 0", 0., 0.001);
	...
	Define(pPs, "Langmuir max", 10., NO_UNIT, "Phosphorus > 0", 5.E-6, 1.E4, 3., 12.); 

	pPs = new Ps(&Langmuir_c50, TOTPROFILE, MINERAL_P, ADSORPTION,NOBPROC, NORMAL); ps_vec.push_back(pPs);
	Define(pPs, "Langmuir half sat", 1.E-3, NO_UNIT, "Phosphorus > 0", 0., 10.); 
	...
	pPs = new Ps(&Psurf_ThresFlow, SINGLE, MINERAL_P, SURFACERUNOFF, NOBPROC, NORMAL); ps_vec.push_back(pPs);
		Define(pPs, "Psurf_ThresFlow", 10., WATERFLOW_UNIT, "Phosphorus > 0", 0., 100.); 
...
// Initial conditions 
	...
	pPs = new Ps(&InitMineral_P_Content, TOTPROFILE, MINERAL_P, NOFPROC, INIT_P, NORMAL);
		Define(pPs, "Inital P content", 0.001, MASSDENSITY_UNIT, "Initial Mineral P = 0|Phosphorus > 0", 0.0000001, 0.1, 0.000001, 0.01);
	...
	pP = new P(&InitMineral_P_ContentZ, pNL, SIMPROFILE, MINERAL_P, NOFPROC, INIT_P, NORMAL); initv.assign(pSoil->NL, 0.001);
		Define(pP, pNL, "Inital P content", initv,MASSDENSITY_UNIT , "Initial mineral P = 1|Phosphorus > 0", 0., 10.);
	mTab_InitConcP.push_back(pP);
