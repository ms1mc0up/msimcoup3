...
public:
	AirDensityTempFunction(double reftemp_C, double height) { abstemp = 273.15; RelativePressureHeightFunction f; density = f(height); reftemp = reftemp_C + 273.15; }
	AirDensityTempFunction(double height) { abstemp = 273.15; RelativePressureHeightFunction f; density = f(height); reftemp = 293.15; }
...
public:
	...
	double operator() (double temp) {
		return moleRatio * f_AirDensityTempFunction(temp);
	}
	double operator() (double volumeconc, double temp) {
		return volumeconc * moleRatio * f_AirDensityTempFunction(temp);
	}
private:
	double moleRatio;
...
public:
	...
	double operator() (double AirPoreFraction) {
		if (Simple) return SimpleDiffusion(AirPoreFraction);
		else return ComplexDiffusion(AirPoreFraction);

	}
	double operator() (double AirPoreFraction, double Temp) {
		if (Simple) return SimpleDiffusion(AirPoreFraction) * f_Temp(Temp);
		else return ComplexDiffusion(AirPoreFraction) * f_Temp(Temp);
	}
private:
	double SimpleDiffusion(double AirPoreFraction) {
		return max(D_MinValue, tourtousf * AirPoreFraction * D_Coef);
	}
	double ComplexDiffusion(double AirPoreFraction) {
		if (AirPoreFraction >= D_AirThreshold)
			return D_MinValue + pow((AirPoreFraction - D_AirThreshold), D_PowerCoef) * tourtousf * D_Coef;
		else
			return D_MinValue;
	}
