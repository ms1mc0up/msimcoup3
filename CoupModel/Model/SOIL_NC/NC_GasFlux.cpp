...
//Use Hydraulic, Only	: M_Saturation
		for(size_t i=0; i<Init_Oxygen_Conc.size(); i++) {
			...
			Oxygen_Conc[i]=Init_Oxygen_Conc[i];
			...
			if (!m_pModelInfo->StartStateValue) {
				Oxygen[i] = Oxygen_Conc[i] * Air_Porosity[i] * p_Soil_WaterF->AThicknessLayers[i] * 
			Fix::MOL_RATIO_O2_AIR*AirDensityTempFunc(pSoilHeat->Temp[i]) / 1.E4; //kg/m-2
...
double OxidationRatio;
	...
	for(size_t i=0; i<Init_Oxygen_Conc.size(); i++) {
		...
		Oxygen_Consumption.back() = .0;  //test boundary condition wenxin
	...
	DO2_SoilCoef.front() = f_OxygenDiffuctionCoef(Air_Porosity.front(), pSoilHeat->Temp.front()) * 86.4E6;
	DCO2_SoilCoef.front() = f_CarbonDiOxideDiffuctionCoef(Air_Porosity.front(), pSoilHeat->Temp.front()) * 86.4E6;

	for(size_t i=1; i<Air_Porosity.size()-1; i++) {
		Mean_Air_Porosity=(Air_Porosity[i-1]+Air_Porosity[i])/2.;
		Mean_Temp=(pSoilHeat->Temp[i-1]+pSoilHeat->Temp[i])/2.;
		DO2_SoilCoef[i] = f_OxygenDiffuctionCoef(Mean_Air_Porosity, Mean_Temp) * 86.4E6;
		DCO2_SoilCoef[i] = f_CarbonDiOxideDiffuctionCoef(Mean_Air_Porosity, Mean_Temp) * 86.4E6;
	}
	// Added bottom layer values Dec 1 PEJ
	DO2_SoilCoef.back() = f_OxygenDiffuctionCoef(Air_Porosity.back(), pSoilHeat->Temp.back()) * 86.4E6;
	DCO2_SoilCoef.back() = f_CarbonDiOxideDiffuctionCoef(Air_Porosity.back(), pSoilHeat->Temp.back()) * 86.4E6;


	if(Oxygen_Model_Sw(Oxygen_Model)== Oxygen_Model_Sw::Steady_state) {
		Oxygen_Flux.back()=0.;

		OxygenMassConcAtm=f_OxygenDensityFrom_Volume_Temp(.2095,pSoilHeat->Temp.front());
		Oxygen_Flux_Infil = Oxygen_Flux.front() + Oxygen_Consumption.front();

		Oxygen.front() = OxygenMassConcAtm - (Oxygen_Flux_Infil * p_Soil_WaterF->AThicknessLayers.front() / 2.)
			/ DO2_SoilCoef.front();

		Oxygen_Conc.front() = Oxygen.front() / f_OxygenDensityFrom_Temp(pSoilHeat->Temp.front());
		...
		for(size_t i=1;i<Air_Porosity.size(); i++) {
			Oxygen[i]-=Oxygen_Flux[i-1]*p_Soil_WaterF->DistanceMidpLayers[i] /DO2_SoilCoef[i];
			Oxygen_Conc[i] = Oxygen[i] / f_OxygenDensityFrom_Temp(pSoilHeat->Temp[i]);
		}
	...
	if (CO2_SoilModel_Sw(CO2_SoilModel) == CO2_SoilModel_Sw::Steady_state) {
		...
		CO2Soil.front() = CO2MassConcAtm + (CO2_Flux_Out * p_Soil_WaterF->AThicknessLayers.front() / 2.) / DO2_SoilCoef.front(); //Mass Per volume of soil.

		CO2_Conc.front() =1.E6*CO2Soil.front() / f_CarbonDiOxideDensityFrom_Temp(pSoilHeat->Temp.front())/ Air_Porosity.front();//Per volume of Air filled soil

		//Do i=2,NL,1
		for (size_t i = 1; i<Air_Porosity.size(); i++) {
			CO2Soil[i] = CO2Soil[i - 1] + (CO2_FluxSoil[i - 1]*p_Soil_WaterF->DistanceMidpLayers[i]) / DCO2_SoilCoef[i]; //Per volume of soil.
			CO2_Conc[i] = CO2Soil[i] / f_CarbonDiOxideDensityFrom_Temp(pSoilHeat->Temp[i])/Air_Porosity[i]; //Per volume of Air filled - Assuming no CO2 into water
		}
		for (size_t i = 1; i < Air_Porosity.size(); i++) {
			CO2_Conc[i] = max(0.0, min(1.E6, 1.E6*CO2_Conc[i]));
