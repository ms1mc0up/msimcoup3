...
/*Use Nc_Plant, only	: SimRootDepth, NUM_NUPT, CRootsLitter, NRootsLitter, KillAboveGroundPlant, KillBelowGroundPlant

Use Plant, only		: RootDistribution, RFracLow
Use Profile, only	: DepthDist, AThicknessLayers
Use NC_Organic, only	: CRootsSoilLitter, NRootsSoilLitter,CRootsSoilOldLitter,NRootsSoilOldLitter
Use NC_Organic, only	: CCoarseRootsSoilLitter, NCoarseRootsSoilLitter,COldCoarseRootsSoilLitter,NOldCoarseRootsSoilLitter

Integer*4		II, OK
Real	Ratio_l1, Ratio_l2, Ratio_hum*/
...
//	Flows from Surface Litter to uppermost soil compartments
	...
	if(int(m_pModelInfo->JDayNum)>0&&(int(m_pModelInfo->JDayNum)==int(SurfaceCultDay)||int(m_pModelInfo->JDayNum)==int(PloughingDay))) {
		...
		for(size_t i=0; i<p_Plant->NumPlants;i++)
			OK=p_NC_Plant->KillAboveGroundPlant(i);

		if(LitterPools>0&&Microbes==0) {
			Ratio_L1=RateCoefSurf_L1/(RateCoefSurf_L1+RateCoefSurf_L2);
			Ratio_L2=1-Ratio_L1;
		}
		else if(Microbes>0) {
			Ratio_L1=RateCoefSurf_L1/(RateCoefSurf_L1+RateCoefSurf_L2+RateCoefSurf_Hum);
			Ratio_L2=RateCoefSurf_L2/(RateCoefSurf_L1+RateCoefSurf_L2+RateCoefSurf_Hum);
			Ratio_Hum=1-Ratio_L1-Ratio_L2;
		}
		else
			Ratio_L1=1.;
		

		SurfaceLitter[_L1][_C_] = Ratio_L1 * SurfaceLitter[_L1][_C_];// *1. / T_Step;
		if(LitterPools>0)	SurfaceLitter[_L2][_C_]	=Ratio_L2*SurfaceLitter[_L2][_C_]*1./T_Step;
		if(Microbes>0)		SurfaceLitter[_H][_C_]	=Ratio_Hum*SurfaceLitter[_L1][_C_]*1./T_Step;


		SurfaceLitter[_L1][_N_] = Ratio_L1 * SurfaceLitter[_L1][_N_];// *1. / T_Step;
		if(LitterPools>0)	SurfaceLitter[_L2][_N_]	=Ratio_L2*SurfaceLitter[_L2][_N_]*1./T_Step;
		if(Microbes>0)		SurfaceLitter[_H][_N_] =Ratio_Hum*SurfaceLitter[_L1][_N_]*1./T_Step;
		Do_Cultivating=true;
	}
