...
public:
	...
	double operator() (double ThetaAngle, double Cloudiness) {
...
class CanopyShadeCover {
public:
	CanopyShadeCover(double CanopyDiameterHeightRatio_Common, double CanopyChadeCoef, vector<double> h_v) : a(CanopyDiameterHeightRatio_Common), b(CanopyChadeCoef), h_vec(h_v) {}
	CanopyShadeCover(double CanopyDiameterHeightRatio_Common, double CanopyChadeCoef) : a(CanopyDiameterHeightRatio_Common), b(CanopyChadeCoef) {}
	double operator() (double ThetaAngle, double height) {
		return ShadeFunc(ThetaAngle, height);
	}
	double operator() (double ThetaAngle, size_t index) {
		if (h_vec.size() > index)
			return ShadeFunc(ThetaAngle, h_vec[index]);
		else
			return MISSING;
	}
private:
	double ShadeFunc(double ThetaAngle, double height) {
		double  Diameter = height*a;
		double elev = Fix::PI / 2. - ThetaAngle;

		if (ThetaAngle < Fix::PI / 2.)
			return  (1.0 + b*height / (Fix::PI*Diameter*tan(elev)));
		else
			return 1.0;
	}
	double a, b;
