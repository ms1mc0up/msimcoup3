 ...
 //   !
  //  !C*************** Global radiation from sunshine ************************
    ...
    if(m_pModelStructure->TimeResolution!=1){  //! Removed 
       ...
       RadInDiffuse=RadInShort*f_DiffuseRadRatio(f_ThetaAngleHour(Hour), CloudFrac);
...
/*          !DG 2001-03-14 CanopyFrac calculated above, since it may be smaller than 
   ...
   // !C*************** NET RADIATION AT SOIL SURFACE ************************
    ...
    if(m_pModelStructure->PlantType==3){ // ! Multiple canopies
       if(LongRadCanopy==0){
		...
		//	 MC_TempCanopy=radout.
            // Call DistributeRad(NumPlants, LAI, PlantHeight, RadNetShort, MC_RntDist, MC_LAIAbove,RadNetGround, RadNetGround1, RadNetGround2)
            ...
            //      , MC_TempCanopy, MC_RntDist, RadNetGround, RadInLongGround,RadInLongGround1,RadInLongGround2,RadNetGround1, RadNetGround2)
		  }
          else {
 ...
 /*         ! END SNOWMIP2 ADDITION, DG 2008-09-02
          ...
          ! COMMENTED BY DG 2008-09-02
          !          Call DistributeRad(NumPlants, LAI, PlantHeight, RadNetShort, MC_RntDist, MC_LAIAbove,RadNetGround, RadNetGround1, RadNetGround2)
          !
          !          Call DistributeLongRad(NumPlants, LAI, PlantHeight, RadInLong,(1.-FracBare)*RadLongOut(TempSnowSurf)	& ! out longwave snow
          !               +FracBare*RadLongOut(TempBareSoil) &
          !               , MC_TempCanopy, MC_RntDist, RadNetGround, RadInLongGround,RadInLongGround1,RadInLongGround2,RadNetGround1, RadNetGround2)
          ! END COMMENT BY DG 2008-09-02

          ! DG 2003-08-18:
          ! The total net radiation has to be re-calculated after DistributeLongRad*/
...
{	RADDIST out;
	...
	for(size_t i=0; i<p_Plant->SoilCoverFraction.size(); i++) {
		...
		if(p_Plant->SoilCoverFraction[i]<1.&&DirectShortWaveSwitch) 
			p_Plant->SoilCoverFraction[i]*=f_CanopyShadeCover(f_ThetaAngleHour(-1), i);
	}
