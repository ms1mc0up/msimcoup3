...
bool Soil_HeatPump_Functions::Def()
{
	...
	Define(pTs, "HeatPumpFlow", 0.,HEATFLOW_UNIT,"Heat Pump >= 1");
	
	Ps *pPs= new Ps(&HPBase, SINGLE, HEAT, EXTRACTION, NOBPROC, NORMAL);
	Define(pPs, "HPBase", 0.,HEATFLOW_UNIT ,	"Heat Pump = 1",-1.E7,1.E7);
	...
	Define(pPs,  "HPCut", -5.,TEMP_UNIT,			"Heat Pump = 1", -50., 20.);
	...
	Define(pPs, "HPMax",   -10.,TEMP_UNIT ,		"Heat Pump = 1",-30., 0.);
	pPs= new Ps(&HPZero  , SINGLE, HEAT, EXTRACTION, NOBPROC, NORMAL);
	Define(pPs, "HPZero",   -10.,TEMP_UNIT ,	"Heat Pump = 1", -100., 10.);
