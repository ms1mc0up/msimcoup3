...
string GetPFCurve::GetComments()
{
	...
	if(comfile.good()){
		...
		for(int i=0;i<totsize;i++){
			if(buf[i]==0x8F) buf[i]= 0xE5; // �
			else if(buf[i]==0x84)	buf[i]=0xE4;		//�
			else if(buf[i]==0x94)	buf[i]=0xF6;		//�
			else if(buf[i]==0xF8)	buf[i]=0xC5;		//�
			else if(buf[i]==0xE8)	buf[i]=0xC4;		//�
			else if(buf[i]==0x99)	buf[i]=0xD6;		//�
