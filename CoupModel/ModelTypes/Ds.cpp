...
Ds::Ds(double *target,enum datatype data, enum elements elem, enum fysprocess fproc, enum bioprocess bproc, enum userlevel ulev)
	:OutSingle(data, elem, fproc, bproc, ulev)
{
	_ptarget=target; 
	SetSimType(DRIVE_SINGLE);
	;
}
void Ds::Def(int ModuleNo, string name, double init, string unit, string conditions)
{
