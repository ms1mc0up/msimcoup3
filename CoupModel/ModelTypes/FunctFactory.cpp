...
function<double(double)> FunctFactory::Set_D1_Functor() {
	...
	if (m_Prop.FunctorInitOption != string::npos) {
		...
		case MyFunc::AIRTEMP_DAILYMEAN:
			if (m_Ps_Par.size() != 2)  return nullptr;
		...
		case MyFunc::RADLONG_OUT_EM:
			if (m_Ps_Par.size() != 1) {  return m_functor1; }
			return RadLongOut_Em(m_Ps_Par[0]);
		case MyFunc::BEER_LAW_SINGLE_LEAF:
			if (m_Ps_Par.size() == 1) return BeerLaw(m_Ps_Par[0]);
			break;
		case MyFunc::RA_BELOWCANOPYLAI:
			if (m_Ps_Par.size() != 1) break;
		...
		case MyFunc::ZADOK_GSI:
			if (m_Ps_Par.size() == 1)
				return ZadokFunctionFromGSI(m_Ps_Par[0]);
			break;
...
function<double(double, double)> FunctFactory::Set_D2_Functor() {
	switch (m_Prop.FuncName) {
		...
		case MyFunc::RADNETLONG_SATURATED_AIR:
			if (m_Ps_Par.size() == 4) 	return RadNetLong(m_Ps_Par[0], m_Ps_Par[1], m_Ps_Par[2], m_Ps_Par[3]);
			break;
		case MyFunc::RADLONGIN_KONZELMANN_EMISSIVITY:
			if (m_Ps_Par.size() == 3) 	return RadLongIn(0, true, m_Ps_Par[0], m_Ps_Par[1], m_Ps_Par[2]);
			break;
		case MyFunc::RADLONGIN_SATTERLUND_EMISSIVITY:
			if (m_Ps_Par.size() == 2) 	return RadLongIn(1, true, m_Ps_Par[0], m_Ps_Par[1]);
			break;
		case MyFunc::RADLONGIN_BRUNT_EMISSIVITY:
			if (m_Ps_Par.size() == 3) 	return RadLongIn(2, true, m_Ps_Par[0], m_Ps_Par[1], m_Ps_Par[2]);
			break;
		case MyFunc::RADLONGIN_KONZELMANN:
			if (m_Ps_Par.size() == 3) return RadLongIn(0, false, m_Ps_Par[0], m_Ps_Par[1], m_Ps_Par[2]);
			break;
		case MyFunc::RADLONGIN_SATTERLUND:
			if (m_Ps_Par.size() == 2) return RadLongIn(1, false, m_Ps_Par[0], m_Ps_Par[1]);
			break;
		case MyFunc::RADLONGIN_BRUNT:
			if (m_Ps_Par.size() == 3) return RadLongIn(2, false, m_Ps_Par[0], m_Ps_Par[1], m_Ps_Par[2]);
			break;
		case MyFunc::ALBEDO_SNOW_TAIR:
			if (m_Ps_Par.size() == 5) return AlbedoSnow_Tair(m_Ps_Par[0], m_Ps_Par[1], m_Ps_Par[2], m_Ps_Par[3], m_Ps_Par[4]);
			break;
		case MyFunc::ALBEDO_SNOW_TSUMAIR:
			if (m_Ps_Par.size() == 4) return AlbedoSnowAgeTempSum(m_Ps_Par[0], m_Ps_Par[1], m_Ps_Par[2], m_Ps_Par[3]);
			break;
		...
		case MyFunc::RAD_RATIO_DIFFUSE:
			if (m_Ps_Par.size() == 1) return DiffuseRadRatio(m_Ps_Par[0]);
			break;
		case MyFunc::CANOPY_SHADE_COVER:
			if (m_Ps_Par.size() == 2) 	return CanopyShadeCover(m_Ps_Par[0], m_Ps_Par[1]);
			break;
		case MyFunc::SOIL_SURFACE_RES_1P:
			if (m_Ps_Par.size() == 1) return SurfaceResistanceSoilSurface_1Par(m_Ps_Par[0]);
			break;
		case MyFunc::SOIL_SURFACE_RES_3P:
			if (m_Ps_Par.size() == 3) return SurfaceResistanceSoilSurface_3Par(m_Ps_Par[0],m_Ps_Par[1], m_Ps_Par[2]);
			break;
		case MyFunc::RA_SNOW_NEUTRAL:
		case MyFunc::RA_HEATSOIL_NEUTRAL:
			if (m_Ps_Par.size() == 3) return ResAirSoilNeutral(m_Ps_Par[0], m_Ps_Par[1], m_Ps_Par[2]);
			break;
		case MyFunc::ROADICE_SALTMELT:
			if (m_Ps_Par.size() == 1) return SaltMeltOnRoad(m_Ps_Par[0]);
			break;
...
function<double(double, double, double)> FunctFactory::Set_D3_Functor() {
	...
	case MyFunc::REL_HUMIDITY_GAS_EQUILIBRIUM:
		if (m_Ps_Par.size() != 1) {  return m_functor3; }
...
function<double(size_t, double)> FunctFactory::Set_D1_TableFunctor() {
		switch (m_Prop.FuncName) {
			...
			case::MyFunc::ALLO_ROOT_MASS_LINEAR:
				if (m_P_Par.size() == 2) {
					return CAlloRootMass_Linear(m_P_Par[0], m_P_Par[1]);
				}
...
function<double(size_t, double, double)> FunctFactory::Set_D2_TableFunctor() {
	...
	case MyFunc::THERMALCOND_BALLARD_ARP:
		if (m_P_Par.size() == 10)
