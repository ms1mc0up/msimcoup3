...
string ModelFiles::GetOutputFileName(size_t ValidIndex, bool LastFile, bool MultiModeFile, size_t MeanPGType, size_t MultiIndex, bool NewFile)
{
	...
	if (ipos_sim - iposext == 4) Old = true;
	if (IsMultiSimulation() && LastFile&&ValidIndex == 0) {
		...
		//size_t in=m_DocFile.m_SimulationRunNo+1-actRunNo;
		//if(in<0) in+=99999;

		//GetFirstRunNoAddedRuns(size_t index)
 ...
 /*   cout <<"MeanPG Type: " << MeanPGType << "\n";
    cout <<"Dir: " << dir << "\n";
	if (MeanPGType == 0) {
        string UserDirectory = "";
        UserDirectory = FUtil::GetProfileStringStd("UserDirectory", UserDirectory);
		stdbinfilename = UserDirectory + "COUP_" + str + ".Bin";
        
        cout <<"coup file name: " << stdbinfilename << "\n";
	}*/
