...
{	int choice;
	...
	CMainFrame *p_mainframe=MySingle::instance().GetFramePointer();
...
bool Sim::CheckAndUpdateFileName(bool MultiRun) {
	...
	size_t iposExt, iposMul, iposSim;
	...
	if (MultiRun) {
		...
		if (iposMul != string::npos&&iposExt != string::npos)
			newfilename = oldfilename.substr(0, iposMul + 7) + NumStr + ".Sim";
		else if (iposExt != string::npos)
			newfilename = oldfilename.substr(0, iposExt + 1) + "Multi_"+ NumStr + ".Sim";
		else if (iposSim != string::npos)
			newfilename = oldfilename.substr(0, iposSim) + "_" +"_Multi_"+ NumStr + ".Sim";
		else
		...
		if (iposMul != string::npos&&iposExt != string::npos)
			newfilename = oldfilename.substr(0, iposMul + 1) + NumStr + ".Sim";
		else if (iposExt != string::npos)
			newfilename = oldfilename.substr(0, iposExt + 1) + NumStr + ".Sim";
		else if (iposSim != string::npos)
			newfilename = oldfilename.substr(0, iposSim) + "_" + NumStr + ".Sim";
		else
			newfilename = oldfilename;
	}
...
bool Sim::CreateNewDocFromCurrentDoc() {
...
#ifdef COUPSTD
	m_DocFile.m_SimulationRunNo = FUtil::GetProfileIntNo("SimulationRunNo");
	PreviousRunNo = 0;
#else
	...
	if (iposext > 0) {
		...
		if (iposMul == string::npos&&m_MultiRun_Array.size() > 0) {
			...
			newPathName = pzPathName.substr(0, iposext + 1) +"Multi_"+ Num + ".Sim";
		...
		newPathName = pzPathName.substr(0, ipos_sim) + "_" + Num + ".Sim";
	else {
		int ipos = pzPathName.size();
		newPathName = pzPathName.substr(0, ipos) + "_" + Num + ".Sim";
	}
	...
	return true;
/*	if (!m_pDoc->OnSaveDocument(newPathName.c_str())) return;
	SetFinishedSimulation();
	if (newPathName != pzPathName) m_DocFile.m_SimulationRunNo = PreviousRunNo;

	m_CurrentFile = string(pzPathName);
	m_DocFile2.m_OriginFileName = string(OldOriginFileName);
	//if (!m_pDoc->CDocument::OnSaveDocument(pzPathName)) return;

	//::ReadResultsFromFile=FALSE;
	m_created_pathname = newPathName;
	pApp->m_pDocTemplate->OpenDocumentFile(newPathName.c_str());
	*/
