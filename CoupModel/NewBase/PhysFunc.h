...
enum class FUNC_CHOICE {
	AIRDENSITYTEMP, SVP, SVP_DAVID, DSVP, CONC_VAPOUR, CASUN_INIT, SUNDAY_INIT, THETA_ANGLE,
	POTRISDAILY, POTRISHOUR, DAYLENGTH, SLOPE_CORR, ATM_CORR, TEMP_VPS, WETBULB, PM, PM_RAD, PM_VENT, AVP_TEMP_RH,
	SHAWP_ROUGHL, SHAWP_DISPL, RH_PSI, D_VAP,D_CO2,D_O2, D_NO, DGAS_TEMP ,
SENSHEAT_AIRFLUX,LATENTHEAT_AIRFLUX, RES_AIR_COM, RES_AIR_MOM_HEAT,
STABILITY_HEAT, STABILITY_MOM, 
MONIN_OBUDKOV_PAR_RICHARD, FRICTION_VELOCITY, RES_AIR_HEAT_GENERAL, WINDPROFILE, RADLONG_OUT_DYN_E, RADLONG_OUT, AIRTEMP_DAY, AIRTEMP_YEAR,
SOILTEMP_ANALYTICAL_DAY, SOILTEMP_ANALYTICAL_YEAR, STABILITY_ANALYTICAL
};

class PlantSeasonDev {
public: PlantSeasonDev(struct PlantInterp L) : L(L) {}
	  double operator() (size_t index, double day) {
		  ...
		  if (day < L.Opt[index]) {
			  ...
			  return L.EndValue[index];
	  }
private:
	struct PlantInterp L;
};
...
public:
	Physf(){
		_a= _b=_c=_d=0.;
		OPTION=FUNC_CHOICE::SVP;
		SNLT=CSLT=SSIN=CCOS=DEC=-1.;
		INIT_CASUN=0;
		SLOPE_CORR=false;
		ICountErr= m_maxitt_monim=0;
		Solar_Time_Adjust= px= py= m_Altitude= m_Angstrom1= m_Angstrom2= m_Hour= m_ThetaAngle= m_DampingDepth=0.;
	...
	Physf(enum  FUNC_CHOICE x, double a, double b, double c, double d) : OPTION(x), _a(a), _b(b), _c(c), _d(d) {
		Physf();
		m_Altitude = _b;
		m_Angstrom1 = _c;
		m_Angstrom2 = _d;
	...
	Physf(enum  FUNC_CHOICE x, double a, double b, double c) : OPTION(x), _a(a), _b(b), _c(c) {
		Physf();
		if (x == FUNC_CHOICE::AIRTEMP_YEAR) {

		}
		ICountErr = 0;
	}
	Physf(enum  FUNC_CHOICE x, double a) : OPTION(x), _a(a) {
		Physf();
		if (x == FUNC_CHOICE::SOILTEMP_ANALYTICAL_DAY || x == FUNC_CHOICE::SOILTEMP_ANALYTICAL_YEAR) {
			m_DampingDepth = a;
		}
		else if (x == FUNC_CHOICE::MONIN_OBUDKOV_PAR_RICHARD) {
			m_maxitt_monim = int(a);
		}
		else {
			SNLT = sin(Fix::RAD*a);
			CSLT = cos(Fix::RAD*a);
	...
	Physf(enum  FUNC_CHOICE x, double a, double b) : OPTION(x), _a(a) {
		Physf();
		if (x == FUNC_CHOICE::SOILTEMP_ANALYTICAL_DAY || x == FUNC_CHOICE::SOILTEMP_ANALYTICAL_YEAR) {
			m_DampingDepth = a;
		}
	...
	double operator() (double x) {
		switch(OPTION) {
		case FUNC_CHOICE::AIRTEMP_YEAR:  // d= shift in days of year
			return _a - _b*cos(2 * Fix::PI*(x - _c) / 365.25);
		case FUNC_CHOICE::AIRTEMP_DAY:  // d= shift in days of year
			return _a - _b*cos(2 * Fix::PI*(x - _c) / 1.00);
		
		case FUNC_CHOICE::AIRDENSITYTEMP:
			return Fix::DENSA*293.15 / (273.15 + x);
		case FUNC_CHOICE::SVP: //Pa
                return ModelFunctions::SVP(x);
		case FUNC_CHOICE::SVP_DAVID: 
			return 613.68*exp((22.452*(x)) / (x + 273.15 - 0.61));
		case FUNC_CHOICE::DSVP:
                return ModelFunctions::DSVP(x);
		...
		case FUNC_CHOICE::RADLONG_OUT: // TempSurf = x, Emisisivity = y
			return 86400. * Fix::STEFB*pow(x + 273.15, 4);
		...
		case FUNC_CHOICE::STABILITY_MOM: 
				...
				if (x <= 0) {
					x = pow(1. - 19.*x, 0.25);   //    ! 19 from H�gstr�m, 1996
					...
					!	if(func.lt.1)then
					!		psim=-5.3*xi  ! 5.3 from H�gstr�m, 1996
					!	else
		...
		case FUNC_CHOICE::STABILITY_HEAT:
			if (x <= 0) {
				x = pow((1. - 11.6*x), 0.25);  //   ! 11.6 from H�gstr�m, 1996
				return 2.*log((1. + x*x) / 2.);
			}
			else {
				/*! Stable case, func=0, Paulsen 1970
				!if(func.lt.1)then
				!	psih=-8.*xi ! 8 from H�gstr�m, 1996
				!else
	...
	double operator() (double x, double y) { 
		...
		case FUNC_CHOICE::AVP_TEMP_RH: //Temp x, RH = y
                return ModelFunctions::SVP(x) *min(1., y / 100.);
		}
		return x;
	}
	...
	double operator() (double a, double b, double  c, double d) {
		if (OPTION == FUNC_CHOICE::SHAWP_DISPL) { //a=CandDensMax, b=PAI, c=RefHeight, d=CanopyHeight
			...
			if (b <= c || b <= d) {
				...
				return max(0.01,log(b / c)*log(b / d)) / (Fix::KARMAN*Fix::KARMAN*a);
	...
	double operator() (double a, double b, double c, double d, double e) {
		...
		else if (OPTION == FUNC_CHOICE::PM) {
            return (ModelFunctions::DSVP(a)*(b) / Fix::LATE + 86400.*Fix::DENSA*Fix::SPECA
				*c / d / Fix::LATE) / (ModelFunctions::DSVP(a) + Fix::GAMMA*(1 + e / d));
		}
		else
			return MISSING;
	}
	double operator() (double TAir, double RadAvail, double VapourPDef, double ResAir, double ResSurf, double LAI) {
		switch (OPTION) {
			case FUNC_CHOICE::PM:
                return (ModelFunctions::DSVP(TAir)*(RadAvail) / Fix::LATE + 86400.*min(1., LAI)*Fix::DENSA*Fix::SPECA
				*VapourPDef / ResAir / Fix::LATE) / (ModelFunctions::DSVP(TAir) + Fix::GAMMA*(1 + ResSurf / ResAir));
			case FUNC_CHOICE::PM_RAD:
				return (ModelFunctions::DSVP(TAir)*(RadAvail) / Fix::LATE) / (ModelFunctions::DSVP(TAir) + Fix::GAMMA*(1 + ResSurf / ResAir));
			case FUNC_CHOICE::PM_VENT:
				return (86400.*min(1., LAI)*Fix::DENSA*Fix::SPECA*VapourPDef / ResAir / Fix::LATE) / (ModelFunctions::DSVP(TAir) + Fix::GAMMA*(1 + ResSurf / ResAir));
		}
		return TAir;
	}
...
private:
	double _psi_m(double xi) {
		//! UNSTABLE CASE FROM PAULSEN ET AL 1970
		if (xi <= 0) {
			double x = pow(1. - 19.*xi, 0.25);   //    ! 19 from H�gstr�m, 1996
			return log(pow((1. + x), 2)*(1. + x*x) / 8.) - 2 * atan(x) + Fix::PI / 2.;
		}
		else {
			/*!
	...
	double _psi_h(double xi) {
		if (xi <= 0) {
			double x = pow((1. - 11.6*xi), 0.25);  //   ! 11.6 from H�gstr�m, 1996
			return 2.*log((1. + x*x) / 2.);
		}
		else {
			/*! Stable case, func=0, Paulsen 1970
