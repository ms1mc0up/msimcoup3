...
string MR::MR_GetExtraLabel(size_t index)
{
	switch (index) {
							...
							case 6:
								...
								break;
							case 7:
								return "Mean Frac Sim";
								break;
							case 8:
								return "Mean Frac Post Sim";
								break;
							case 9:
								return "Mean Frac Inter Sim";
								break;
						}
	return "";
...
{  	
	...
	size_t	ii, irep, icount, nTabIndex, nTabIndexDep, icountBayes ;
	...
	//for(size_t i=0;i<MR_GetNumberOfDimensions();i++){
		...
		for (size_t i = 0; i < m_ActualDimension + 1; i++) {
			...
			//}
				...
				if (MR_GetMethod(i + 1, ii + 1) != 5 && MR_GetMethod(i + 1, ii + 1) <= 9 || MR_GetMethod(i + 1, ii + 1) == 11) {
...
void MR::MR_CHECK_Bayesian(bool UpdateExt)
{
	...
	if(!m_IsMakingBayesianCalib) return;
...
// Apply the covariance matrice as estimated from previous run
		Num_BayesPar=MR_Get_NumBayesPar();
...
void MR::MR_Storage_Init()
{
	...
	auto ipos_sim=pzPathName.find(".Sim");
	if (ipos_sim<0)
...
void MR::MR_Storage_Add()
{
	...
	indexp++; //+6
	m_MStorage.AddMultiP(irec,indexp, m_DocFile.m_SimTime);
		indexp++; //+7
	m_MStorage.AddMultiP(irec,indexp, m_DocFile.m_AccSimTime);
		indexp++; //+8
	m_MStorage.AddMultiP(irec,indexp, 0./* TIMEINFO_mp_FRACSIM*/);
		indexp++; //+9
	m_MStorage.AddMultiP(irec,indexp, 0./* TIMEINFO_mp_FRACPOST*/);
		indexp++; //+10
	//m_MStorage.AddMultiP(irec,indexp, TIMEINFO_mp_FRACINTER);
	m_MStorage.AddMultiP(irec,indexp, 0.);
...
void MR::MR_ReCalculatePostDist(bool BayesMeanMethod)
{
	...
	m_AcceptedParDist.resize(MR_Get_NumBayesPar());
	for(size_t i=0;i<MR_Get_NumBayesPar();i++) {
		vector<float> values;
		...
		for(size_t ii=0;ii<naccepted;ii++) {
				...
				if(irun>GetNumBurnIn()&&BayesMeanMethod) {
					...
					auto fval=m_MStorage.GetMultiP(irun,i);
