...
void* ModelBase::GetNE_Ptr(size_t* val) {
	map<size_t*, void*>::iterator it;
	it=MapNE.find(val);
	if(it!=MapNE.end())
		return it->second;
	return nullptr;
}
vector<SimB*> ModelBase::GetPtrVector(enum simtype typ,  bool All)
{
	SimB *pSimB;
	vector<SimB*> vptr;
...
vector<SimB*> ModelBase::GetPtrVector(enum simtype typ,size_t GroupNo,  bool All)
{
	SimB *pSimB;
...
vector<SimB*> ModelBase::GetAllPtr(enum simtype typ) {
	vector<SimB*> vptr;
	SimB* pSimB;
	for (auto it = GlobalMap.begin(); it != GlobalMap.end(); ++it) {
		pSimB = (*it).second;
		if(pSimB->GetGroupNo()>3&& pSimB->GetGroupNo() < 38&& typ == pSimB->GetType())
			vptr.push_back(pSimB);
	}
	return vptr;
}
...
vector<SIMB> ModelBase::GetAllOutputs(size_t choice) {
	vector<SIMB> out, allout;
	vector<SimB*> vp;
...
float ModelBase::GetActualObsValueFromValSumFlex(string IDFileName, string  IDVarName, size_t runno) {
	...
	index_var = 0;
	auto it = MapFlexId[index_file].find(IDVarName);
